...
def weaponGen():
    ...
    #Prefix
    prefix_list = ['Hairy|2|', 'Sharp|4|', 'Acute|3|', 'Apical|3|', 'Serrated|3|', 'Dull|1|', 'Dense|2|', "Roelofs|5|", 'Solid|2|', 'Micro|3|', 'Wet|5|', 'Meme|2|', 'Thick|4|', 'Shitty|1|', 'Primed|3|', "BullyHunters|6|", "Some Cunts|5|", 'Soft|1|', 'Whale-Killing|7|']
...
{} **Prefix Multiplier:** {}\n\
{} **Resale Value:** {} Tokens\
".format(name, part[3], emoji_1, u'\U0001F397', part[2], u'\U0001F48D', part[4], emoji_2, u'\U0001F4FF', part[5], u'\U0001F3AF', part[7], part[6], u'\U0001F4A5', part[1], u'\U0001F39B', part[8])
    return weapon
...
async def on_ready():
    ...
    with open("GameData.json", "r+") as file:
        data = json.loads(open('GameData.json').read())
        await client.send_message(client.get_channel("498311009635794964"), "[DYNO RESTART] Synced From Google Drive Successfully.\n{}".format(str(data)))
...
async def on_message(message):
    ...
    k = False
    while k == True:
        listo = ['**Happy Birthday Aidan**', "**Did you know it is Aidan's Birtday today?**", "**Aidan turned 15 today**", "**Fuck the what**", "**Aidan was born today**"]
    ...
    if message.author == client.user:
        ...
        if message.author.id in StopPermList:
            ...
            if len(syntax) < 2:
                await client.send_message(message.channel, "[Syntax Error] Clean Requirement is not specified. E.g '$clean ThisIsSpam 100'")
            elif len(syntax) == 2:
                limit = 25
            else:
                try:
                    await client.send_message(client.get_channel("493017783135764486"), "Clean Request from {}. {}".format(message.server.name, syntax))
                    limit = round(int(syntax[2]))
                except:
                    await client.send_message(message.channel, "[Syntax Error] Clean limit is invalid. Must be numbers only. E.g '$clean ThisIsSpam 100'")
            req = syntax[1]
            counter = 0
            ...
            await client.send_message(client.get_channel("493017783135764486"), "{} Messages Searched for '{}', {} Messages have been cleaned.".format(limit, req, counter))
            await client.send_message(message.channel, "{} Messages Searched for '{}', {} Messages have been cleaned.".format(limit, req, counter))
            ...
            if syntax[1] == 'daily':
                with open("GameData.json", "r+") as file:
                    ...
                    try:
                        ...
                        if abs(datetime.strptime(match["dailytime"][:10], '%Y-%m-%d').date() - date.today()).days >= 1:
                            ...
                            file.truncate()
                            sync()
                            await client.send_message(message.channel, '\U0001F4B0 | **{} has received 200 tokens**'.format(message.author.name))
                        else:
                            await client.send_message(message.channel, '\U0001F550 | {} daily rewards can only be claimed every 24 hours. (Last Claimed: {})'.format(message.author.name, match["dailytime"][:15]))
                    except StopIteration:
                        NewPlayer = PlayerCreation(message.author.id, None, 200, str(datetime.today()), str(datetime.today() - timedelta(days=2)), None)
                        data["Players"].append(NewPlayer)
                        with open('GameData.json', 'w') as file:
                            file.write(json.dumps(data, default=jdefault, indent=2))
                        sync()
                        await client.send_message(client.get_channel("498311009635794964"), "Synced To Google Drive Successfully.\n{}".format(str(data)))
                        await client.send_message(message.channel, '\U0001F4B0 | **{} has received 200 tokens**'.format(message.author.name))
                        
            elif syntax[1] == 'tokens':
                with open("GameData.json", "r+") as file:
                    data = json.loads(open('GameData.json').read())
                    try:
                        match = next(p for p in data['Players'] if p['id'] == message.author.id)
                        await client.send_message(message.channel, '\U0001F3E6 | **{}, you currently have {} tokens in your bank account**'.format(message.author.name, match["bank"]))
                    except StopIteration:
                        NewPlayer = PlayerCreation(message.author.id, None, 0, str(datetime.today() - timedelta(days=2)), str(datetime.today() - timedelta(days=2)), None)
                        data["Players"].append(NewPlayer)
                        with open('GameData.json', 'w') as file:
                            file.write(json.dumps(data, default=jdefault, indent=2))
                        sync()
                        await client.send_message(client.get_channel("498311009635794964"), "Synced To Google Drive Successfully.\n{}".format(str(data)))
                        await client.send_message(message.channel, '\U0001F3E6 | **{}, you currently have 300 tokens in your bank account**'.format(message.author.name))
                    ...
                    try:
                        ...
                        if len(syntax) > 2:
                            if syntax[2] == 'buy':
                                if match['bank'] >= 500:
                                    if match['wep'] == None:
                                        await client.send_message(message.channel, "\U0001F4C3 | **Buying a new weapon costs 500 tokens. Would you still like to continue? (React to emoji)**")
                                        buy_confirm_list.append(message.author.id)    
                                        await client.add_reaction(message, u"\u2705")
                                        await client.add_reaction(message, u"\u274E")
                                    else:
                                        await client.send_message(message.channel, "\U0001F4C3 | **{} You cannot have 2 weapons at once. If you buy a new weapon you will need to sell one. Would you still like to continue? (React to emoji)**".format(message.author.name))
                                        buy_confirm_list.append(message.author.id)    
                                        await client.add_reaction(message, u"\u2705")
                                        await client.add_reaction(message, u"\u274E")
                                else:
                                    await client.send_message(message.channel, "\U0001F4C3 | **Buying a new weapon costs 500 tokens. You only have {} Tokens! (Maybe Try $owl daily)**".format(match['bank']))
                                    
                            elif syntax[2] == 'sell':
                                if match['wep'] == None:
                                    await client.send_message(message.channel, "\U0001F4DC | **{}, you currently do not have a weapon! Use '$owl weapon buy' to buy one**".format(message.author.name))
                                else:
                                    await client.send_message(message.channel, "\U0001F4C3 | **Would you really like to sell your weapon? (React to emoji)** \n**Your Current Weapon:**\n {}".format(formatter(match['wep'], message.author.name)))
                            ...
                            if match['wep'] == None:
                                ...
                                await client.send_message(message.channel, "{}".format(formatter(match["wep"], message.author.name)))
                ...
                with open("GameData.json", "r+") as file:
                    ...
                    try:
                        match = next(p for p in data['Players'] if p['id'] == message.author.id)
                        if match['wep'] == None:
                            await client.send_message(message.channel, "{}, you currently do not have a weapon to fight! Use '$owl weapon buy' to buy one".format(message.author.name))
                        else:
                            if abs(datetime.strptime(match["fighttime"][:10], '%Y-%m-%d').date() - date.today()).days >= 1:
                                enemy_list = [' Sheep', ' Owl', ' Coffee', ' Lucy', ' Dude', ' Ricegum', ' Jake Paul', ' Goose', ' Knight', ' Panda', ' Roelof', ' Whale', ' Elephant']
                                enemy = weaponGen()
...
Would you like to **[RUN AWAY]**(Does not Consume Daily Charge) or **[FIGHT]**(Consumes daily charge)?".format(message.author.name, part[0]+type, u'\U000026A0', emoji_2, u'\U0001F497', part[6], u'\U0001F4B0', part[8]))
                                ...
                                await client.send_message(message.channel, '\U0001F550 | **{}, you can only fight once every 24 hours. (Last Fight: {})**'.format(message.author.name, match["fighttime"][:15]))
                    except StopIteration:
                        NewPlayer = PlayerCreation(message.author.id, None, 0, str(datetime.today() - timedelta(days=2)), str(datetime.today() - timedelta(days=2)), None)
                        data["Players"].append(NewPlayer)
                        with open('GameData.json', 'w') as file:
                        ...
                        sync()
                        await client.send_message(client.get_channel("498311009635794964"), "Synced To Google Drive Successfully.\n{}".format(str(data)))
                        await client.send_message(message.channel, "\U0001F4DC | **{}, you currently do not have a weapon to fight! Use '$owl weapon buy' to buy one**".format(message.author.name))
        else:
            await client.send_message(message.channel, "\U000026A0 | [Syntax Error] $owl command must have atleast 1 syntax! ($help for more Information)")
            ...
            if role.lower() in role_names:
                ...
                for r in message.author.roles:
                    if r.id == roles[proper_position]:
                        await client.send_message(message.channel, "You already possess this role {}".format('<@' + str(message.author.id) + '>'))
                        role_match = True
                        return
                if role_match == False:
                    try:
                        ...
                        await client.send_message(message.channel, "Successfully added role {} to {}".format(role_proper_names[proper_position], '<@' + str(message.author.id) + '>'))
                    except discord.Forbidden:
                        await client.send_message(message.channel, "Insufficient Permissions for adding roles")
            else:
                await client.send_message(message.channel, "No matching role Found {}".format('<@' + str(message.author.id) + '>'))
                ...
                for r in message.author.roles:
                    if r.id == roles[proper_position]:
                        ...
                        try:
                            ...
                            await client.send_message(message.channel, "Successfully removed role {} from {}".format(role_proper_names[proper_position], '<@' + str(message.author.id) + '>'))
                        except discord.Forbidden:
                            await client.send_message(message.channel, "Insufficient Permissions for removing roles")
                if role_match == False:
                    await client.send_message(message.channel, "You do possess this role {}".format('<@' + str(message.author.id) + '>'))
                    return
            else:
                await client.send_message(message.channel, "No matching role Found {}".format('<@' + str(message.author.id) + '>'))
        else:
        ...
        if user.id in buy_confirm_list:
            if reaction.emoji == u"\u2705":
                ...
                if match['wep'] == None:
                    ...
                    sync()
                    await client.send_message(client.get_channel("498311009635794964"), "Synced To Google Drive Successfully.\n{}".format(str(data)))
                    await client.send_message(reaction.message.channel, "\U0001F4EC | **{} has received a new weapon!**\n{}".format(user.name, formatter(match['temp_wep'], user.name)))
                else:
                    await client.send_message(reaction.message.channel, "\U0001F6AB | **You cannot have 2 weapons at once! You will need to choose which to keep. (React to Emoji)**\n**__1. Original Weapon__**\n{}\n\n**__2. New Weapon__**\n{}".format(formatter(match["wep"], user.name), formatter(match["temp_wep"], user.name)))
            ...
            if reaction.emoji == u"\u2705":
                ...
                await client.send_message(reaction.message.channel, "\U0001F4B0 | **{} has sold a weapon for {} Tokens**".format(user.name, part[8]))
            ...
            if reaction.emoji == u'\U00002694': #fight
                ...
                if part[0] == 'W':
                    ...
                    match['bank'] -= int(part[1])
                    await client.send_message(reaction.message.channel, "\U0001F3F3 | **{} has lost a fight and lost {} Tokens. (Maybe try running next time)**".format(user.name, part[1]))
                ...
                fight_confirm_list.remove(user.id)
                await client.send_message(reaction.message.channel, "\U0001F3C3 | **{} Safely ran away**".format(user.name))
            ...
            if reaction.emoji == u'\U0001F195': #new
                ...
                await client.send_message(reaction.message.channel, "\U00002694{} | **{} has sold his old weapon for {} and received a new weapon!**\n{}".format(user.name, sale ,formatter(match['temp_wep'], user.name)))
            elif reaction.emoji == u'\U0001F502': #old
                choice_list.remove(user.id)
                await client.send_message(reaction.message.channel, "\U0001F44D | **{} decides to keep his old weapon**".format(user.name))
